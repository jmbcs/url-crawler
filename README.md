...
make test      # 590 tests, offline and deterministic
...
- **Safety.** robots.txt is on by default and a robots.txt that cannot be read blocks the crawl,
  per RFC 9309. Scope is exact `(host, port)` equality rather than a suffix match, credentials in a
  URL are stripped before it is queued or printed, and bodies stream behind a content-type gate and
  a size cap so a 4 GB video is never downloaded. The service also refuses a seed that resolves to
  a private address; the CLI does not, because your own terminal already reaches those.
...
- **Two runtime dependencies.** The CLI needs `httpx` and `selectolax` and nothing else. The service
  ...
  extra, so `pip install .` still installs exactly those two. The frontier, the scope check, the
  retry policy, the robots.txt handling and the link extraction are written in this repository
  rather than pulled from a crawling framework, which is what makes each of them testable and
  explainable in the docs below.
- **590 tests**, deterministic and offline by default, over a fake site that packs every crawl
